...
impl Mda {
    ...
    fn send_probe(&self, h: u8, phi: u16) -> Option<Ipv4Addr> {
        ...
        let transport_bytes = UdpHeaderBuilder::new()
            .source_port(phi)
            .destination_port(
                UDP_INITIAL_DEST_PORT.wrapping_add(UDP_INITIAL_SRC_PORT.wrapping_sub(phi)),
            )
            .build()
        ...
        loop {
            if self
                .socket
                .send_to(&pkt, &SocketAddr::from((self.destination, 0)).into())
                .is_err()
            {
                return None;
            }
    ...
    pub fn multipath_traceroute(&self, hmin: u8, hmax: u8) -> Vec<Vec<Ipv4Addr>> {
        ...
        for h in hmin..=hmax.saturating_add(1) {
            ...
            for r in &previous_interfaces {
                ...
                let flows_to_r: Vec<u16> = flows_at_hop_h_minus_1
                    .get(r)
                    .cloned()
                    .unwrap_or_default()
                    .into_iter()
                ...
                let (nexthop_interfaces_of_r, flows_to_nexthops) =
                    self.next_hops(h, &flows_to_r, is_source);
